        ...
        <form action="/workspaces/{{ $workspace_id }}/tasks" method="POST" class="mb-10 bg-white p-6 sm:p-8 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-300">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="md:col-span-2 lg:col-span-1">
                    ...
                    <input type="text" name="name" required placeholder="Misal: Revisi Desain Homepage" 
            ...
            <div class="space-y-4">
                @forelse($tasks as $task)
                    <div class="group flex flex-col md:flex-row md:items-center justify-between p-5 sm:p-6 bg-white border border-gray-200 rounded-2xl shadow-sm hover:shadow-md transition-all duration-200 gap-4">
                        ...
                        <div class="flex-1">
                            <h4 class="text-lg font-bold text-gray-900 group-hover:text-blue-700 transition-colors">{{ $task->name }}</h4>
                            ...
                            <div class="flex flex-wrap items-center gap-3 mt-4">
                                ...
                                @if($task->deadline)
                                <span class="inline-flex items-center text-xs font-semibold {{ \Carbon\Carbon::parse($task->deadline)->isPast() && $task->status !== 'completed' ? 'text-red-600 bg-red-50 border-red-200' : 'text-orange-600 bg-orange-50 border-orange-200' }} border px-3 py-1 rounded-full">
                        ...
                        <div class="flex flex-row md:flex-col items-center justify-between md:justify-end gap-3 w-full md:w-48 mt-2 md:mt-0 pt-4 md:pt-0 border-t md:border-0 border-gray-100">
                            ...
                            @php
                                $statusColor = match($task->status) {
                                    'completed', 'done' => 'bg-green-50 text-green-700 border-green-200 focus:ring-green-500',
                                    'in_progress', 'review' => 'bg-blue-50 text-blue-700 border-blue-200 focus:ring-blue-500',
                            ...
                            <select id="status-{{ $task->id }}" onchange="updateTaskStatus({{ $task->id }}, this.value)" 
                                ...
                                <option value="review" {{ $task->status == 'review' || $task->status == 'in_progress' ? 'selected' : '' }}>🚀 Review</option>
